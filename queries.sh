...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams AS T FULL JOIN games AS G ON G.winner_id = T.team_id LEFT JOIN  games AS G2 ON G2.opponent_id = T.team_id WHERE (G2.year=2014 AND G2.round='Eighth-Final') OR (G.year=2014 AND G.round='Eighth-Final') ORDER BY name ASC")"
